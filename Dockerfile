...
EXPOSE 10000/tcp
CMD ["/bin/sh", "-c", "/etc/init.d/webmin start&sed -i -e "/listen/i\\\\tallow-recursion { ${RECURSION_SUBNET}; };" /etc/bind/named.conf.options&&/usr/sbin/named -g -c /etc/bind/named.conf -u bind"]